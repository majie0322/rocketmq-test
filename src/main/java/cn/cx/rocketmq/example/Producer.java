...
import cn.cx.rocketmq.util.ObjectToByteUtil;
...
public class Producer {
	...
	public static void main(String[] args) throws MQClientException, InterruptedException {
		...
		// nameserver服务,多个以;分开,如果没有指定namesrv地址，将会自动寻址
			...
			try {
				{ // 通过topic订阅消息，tag过滤消息
					Message msg = new Message("TopicTest1", // topic
							"TagA", // tag 消息标签，只支持设置一个Tag（服务端消息过滤使用）可为null
							"OrderID001", // key   消息索引
										// 消息关键词，多个Key用KEY_SEPARATOR隔开（查询消息使用）可以用于去重，可为null
...

				{
					Message msg = new Message("TopicTest2", // topic
							"TagB", // tag
							"OrderID0034", // key
							ObjectToByteUtil.ObjectToByte("cx"+i, 23, "coder"));// body
					SendResult sendResult = producer.send(msg);
					System.out.println(sendResult);
				}
